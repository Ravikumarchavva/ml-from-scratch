...
class LinearRegression:
    ...
    Usage:
        model = LinearRegression(learning_rate=0.001, n_iterations=10000, initial_weights='normal')
    ...
    example output:
        Run ```python linear_regression.py``` locally to see the output.
    ...
    def fit(self, X, y, verbose=0):
        ...
        for i in range(self.n_iterations):
            ...
            error = y_pred - y
            self.weights -= self.learning_rate * np.dot(X.T, error) / len(y) # Update weights
            self.bias -= self.learning_rate * np.mean(error)    # Update bias
