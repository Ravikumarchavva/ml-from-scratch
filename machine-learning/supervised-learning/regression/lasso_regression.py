...
class LassoRegression(Model):
    ...
    Usage:
        model = LassoRegression(learning_rate=0.001, n_iterations=10000, l1_penalty=0.01, initial_weights='normal')
    ...
    example output:
        Run ```python lasso_regression.py``` locally to see the output.
    ...
    '''
    def __init__(self, learning_rate=0.01, n_iterations=1000, l1_penalty=0.01, initial_weights='normal'):
    ...
    def fit(self, X, y, verbose=0):
        ...
        if verbose > 0:
            print(f"Mean Squared Error: {np.mean(np.square(error)):,.2f}")
